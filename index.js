...
const app = express();
app.get('/', (req, res) => res.send('ADAM-AIRA V2 IS ALIVE ⚡'));
app.listen(process.env.PORT || 3000);

// --- 🤖 CONFIG ---
const token = '8066622469:AAHfTl75LWgyhMVqvJ61RUH45MWOZJ9bU90'; // Ninte puthiya token
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (!msg.message) return; // Self message-um work aakum!
        ...
        if (cmd === '.ping') {
            await sock.sendMessage(from, { text: "🚀 *Aira Speed:* 12ms" });
        }
        if (cmd === '.bug') {
