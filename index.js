...
const app = express();
app.get('/', (req, res) => res.send('ADAM-AIRA IS LIVE & ON MASS ⚡'));
app.listen(process.env.PORT || 3000);

// --- 🤖 CONFIG ---
const token = '8346342613:AAG3ghHtKoYw2BYGOmfe40oYqR2nnYdKJRI'; 
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (!msg.message) return; // Ithu maatti, inni self messages-um work aakum!
        ...
        if (cmd === '.ping') {
            await sock.sendMessage(from, { text: "🚀 *Aira Speed:* 24ms" });
        }
        if (cmd === '.bug') {
