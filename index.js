...
const app = express();
app.get('/', (req, res) => res.send('ADAM-AIRA BOT IS ALIVE ⚡'));
app.listen(process.env.PORT || 3000);

// --- 🤖 CONFIG ---
const token = '8346342613:AAG3ghHtKoYw2BYGOmfe40oYqR2nnYdKJRI'; // Puthiya Token
...
async function startBot() {
    ...
    bot.on('message', async (msg) => {
        ...
        if (text && text.startsWith('/pair')) {
            ...
            try {
                bot.sendMessage(msg.chat.id, "⏳ *Generating Code for " + num + "...*", { parse_mode: 'Markdown' });
                let code = await sock.requestPairingCode(num.replace(/[^0-9]/g, ''));
                bot.sendMessage(msg.chat.id, `⚡ *YOUR PAIRING CODE:* \n\n \`${code}\` \n\nEnter this in WhatsApp Linked Devices!`, { parse_mode: 'Markdown' });
            } catch (e) {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') bot.sendMessage(chatId, "✅ *Adam-Aira Connected!* \nBot ippo Render-il active aanu! 😈");
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (cmd === '.menu') {
            await sock.sendMessage(from, { text: "⚡ *ADAM-AIRA ON MASS* ⚡\n\n💥 .bug\n🚀 .ping" });
        }
        if (cmd === '.bug') {
